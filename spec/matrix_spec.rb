...
describe Matrix do
  ...
  context "when extracting a row of a matrix" do
    it "returns [1, 2] as the first row of the matrix" do
      matrix = Matrix.new("1 2\n10 20")
      expect(matrix.extract_row(0)).to eq([1, 2])
    end

    it "returns [8, 6] as the second row of the matrix" do
      matrix = Matrix.new("9 7\n8 6")
      expect(matrix.extract_row(1)).to eq([8, 6])
    end

    it "returns [9, 8, 7] as the first row of the matrix" do
  ...
  context "when extracting a column of a matrix" do
    it "returns [1 , 10] as the first column of the matrix" do
      matrix = Matrix.new("1 2\n10 20")
      expect(matrix.extract_column(0)).to eq([1, 10])
    end

    it "returns [5 , 20] as the second column of the matrix" do
      matrix = Matrix.new("4 5\n19 20")
      expect(matrix.extract_column(1)).to eq([5, 20])
    end

    it "returns [19, 13] as the second column of the non-regular matrix" do
